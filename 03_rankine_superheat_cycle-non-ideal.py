...
turbeff = 0.9 # turbine isentropic efficiency can be entered here (typically 0.85 - 0.95)
...
plt.title('T-s Diagram - Rankine Superheat Cycle (non ideal)')
plt.rc('font', **font)
...
plt.plot([s1, s2, s2dash, s3dash, s3, s4r, s1],[T1, T2, T2dash, T3dash, T3, T4, T1], 'black', linewidth=2.0)
...
plt.text(6.3,T3-50,f'(3)\nT = {round(float(T3),2)} C\nh = {round(float(h3),1)} kJ/kg \ns = {round(float(s3),3)} kJ/kgK',
    ha='right',backgroundcolor='white')
plt.text(s4r+.1,T4,f'(4)\nT = {round(float(T4),2)} C\nh = {round(float(h4r),1)} kJ/kg \ns = {round(float(s4r),3)} kJ/kgK \nx = {round(float(x4r),3)}',
    ha='left',backgroundcolor='white')

plt.savefig('03-rankine-superheat-cycle-non-ideal-TSdiagram.png')
